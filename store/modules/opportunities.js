...
export default {
  ...
  mutations: {
    ADD_OPPORTUNITY(state, payload) {
      ...
      payload.map((opportunity) => {
        if (
          !state.all.find(
            (o) => o.links.subscription === opportunity.links.subscription
          )
        ) {
          ...
          state.uniqueTitles.indexOf(opportunity.title) === -1 &&
            state.uniqueTitles.push(opportunity.title);
        }
      });
    },
    ...
    SET_SUBSCRIPTIONS(state, payload) {
      ...
      payload.map((subscription) => {
        const exists = state.subscriptions.findIndex(
    ...
    SET_SPECIFIC_SUBSCRIPTION(state, payload) {
      ...
      const indexToReplace = old_subscriptions.findIndex(
        (s) => s.opportunityId == payload.opportunityId
      );
      if (indexToReplace >= 0) {
  ...
  actions: {
    setOpportunities({ commit, dispatch, rootState }, obj) {
      ...
      if (obj && obj.internal_recruitment) {
        params += '&internal_recruitment=' + obj.internal_recruitment;
      }
      if (obj && obj.page) {
        params += '&page=' + obj.page;
      }
      if (obj && obj.collection) {
        params += '&collection=' + obj.collection;
      }
      if (obj && obj.jobName) {
        params += '&q=' + obj.jobName;
      }
      if (obj && obj.cities && obj.cities.length > 0) {
        params +=
          '&city=' +
          Vue.prototype.$helpers.cleanStringToCompare(
            obj.cities.join('&city=')
          );
      }
      if (obj && obj.pcd) {
        params += '&pcd=' + obj.pcd;
      }
      if (obj && obj.temporary) {
        params += '&temporary=' + obj.temporary;
      }
      if (obj && obj.level && obj.level.length > 0) {
        params +=
          obj.level.constructor === Array
            ? '&level[]=' + obj.level.join('&level[]=')
            : '&level=' + obj.level;
      }
      if (obj && obj.filial) {
        params += '&filial=' + obj.filial;
      }
      if (obj && obj.city) {
        params +=
          '&city=' + Vue.prototype.$helpers.cleanStringToCompare(obj.city);
      }
      if (obj && obj.states) {
        params += '&states=' + obj.states;
      }
      if (obj && obj.neighborhoods) {
        params += '&bairro=' + obj.neighborhoods;
      }
      if (obj && obj.company_field_area && obj.company_field_area.length > 0) {
        params += '&company_field_area=' + obj.company_field_area.join(',');
      }
      if (
        obj &&
        obj.company_field_nivel &&
        obj.company_field_nivel.length > 0
      ) {
        params += '&company_field_nivel=' + obj.company_field_nivel.join(',');
      }

      return new Promise((resolve, reject) => {
        axios
          .get(
            `${Vue.prototype.OPPORTUNITIES_ENDPOINT}/v1/opportunities?${params}`,
            header
          )
          .then(
            (resp) => {
    ...
    getOpportunity({ dispatch, commit, rootState }, id) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .get(
            `${Vue.prototype.OPPORTUNITIES_ENDPOINT}/v1/opportunities/${id}`,
            header
          )
          .then(
            (resp) => {
              ...
              if (opportunity) {
                ...
                opportunity.slug =
                  opportunity_slug[opportunity_slug.length - 1];
                dispatch('isEnrolled', { opportunityId: opportunity.id }).then(
                  (isEnrolled) => {
    ...
    isEnrolled({ rootState }, obj) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post(Vue.prototype.API_ENDPOINT + '/graphql', params, {
            headers: { Authorization: rootState.tkn },
          })
          .then(
            (resp) => {
      ...
      params.append(
        ...
        query {
          subscriptions(
            ...
            ${
              'onlyRI' in rootState
                ? ' internalRecruitment: "' + rootState.onlyRI + '" ,'
                : ''
            }
          )
          {
    ...
    subscriptionsWithPagination({ commit, rootState }, obj = {}) {
      ...
      return new Promise((resolve, reject) => {
        axios
          ...
          .then(
            (resp) => {
              const subscriptionsWithPagination =
                resp.data.data.subscriptionsWithPagination.records;
          ...
          .then(
            (resp) => {
              const subscriptionQuestions =
                resp.data.data.subscriptionQuestions;
    ...
    validateCandidateEliminatoryQuestions({ rootState }, obj) {
      ...
      return new Promise((resolve, reject) => {
        axios
          ...
          .then(
            (resp) => {
              const validateCandidateEliminatoryQuestions =
                resp.data.data.validateCandidateEliminatoryQuestions;
        ...
        mutation {
          subscriptionNew(
            ...
            ${
              obj.indication_form
                ? 'indication: { name: "' +
                  obj.indication_form.name +
                  '", email: "' +
                  obj.indication_form.email +
                  '" }'
                : ''
            }
            ...
            ${
              obj.send_confirmed_email
                ? ' send_confirmed_email: ' + obj.send_confirmed_email + ' ,'
                : ''
            }
            ${
              obj.ignore_online_tests
                ? ' ignore_online_tests: ' + obj.ignore_online_tests + ' ,'
                : ''
            }
          )
          {
        ...
        axios
          ...
          .then(
            (resp) => {
              ...
              if (!response.error) {
                const subscriptionLogicalDelete =
                  response.data['subscriptionLogicalDelete'];
                const currentSubscriptions =
                  rootState.opportunities.subscriptions;
                const foundIndex = currentSubscriptions.findIndex(
                  (x) => x.id == obj.subscription_id
                );

                currentSubscriptions[foundIndex]['deletedAt'] =
                  subscriptionLogicalDelete.deletedAt;
                currentSubscriptions[foundIndex]['isDeleted'] =
                  subscriptionLogicalDelete.isDeleted;
